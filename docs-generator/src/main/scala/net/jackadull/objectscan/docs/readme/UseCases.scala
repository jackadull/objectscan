...
object UseCases extends Chapter {
  def id = "use_cases"
  def title = "Use Cases"
...
<p>
  ...
  override def subChapters = Seq(
    Chapter("how_singletons_are_provided_and_found", "How Singletons are Provided and Found",
<p>
  <tt>objectscan</tt> finds singleton instances.
...
<p>
  This will simply inject the returned <tt>scannableObjects</tt> into the pool of scannable singletons, and behave as if they where defined as Scala <tt>object</tt>, each respectively.
  When a scan is restricted by package prefix, the package prefix of the enclosing <tt>ObjectScanSource</tt> counts for that restriction.
</p>
<p>
  Note that it is possible to use <tt>objectscan</tt> inside a <tt>scannableObjects</tt> implementation.
  However, such cases must be handled with great care:
  <tt>objectscan</tt> does not handle recursions gracefully.
  The developer must make sure that no recursions can occur on a path of <tt>ObjectScanSources</tt> that are linked to each other.
</p>
<p>
  Also take into account that <tt>scannableObjects</tt> <b>must always</b> return the same result, no matter when or how many times it gets called.
  The reason is given below, under the heading "Stability assumption".
</p>
    ),
    Chapter("not_to_be_used_for_di", "Not to be Used for DI",
<p>
  However, <tt>objectscan</tt> is not useful for dependency injection.
...
<p>
  ...
  Instead of hard-wiring to a static instance, you use <tt>objectscan</tt> for finding the single instance of <tt>DatabaseAccess</tt>, and then later on, to use that instance.
...
<p>
  ...
  All of a sudden, those two modules cannot be used together, because they introduce two <tt>DatabaseAccess</tt> singletons to the classpath.
</p>
<p>
  Even though this case could in theory be resolved, by adding your own discriminator methods for resolving scope, there are more arguments why <tt>objectscan</tt> is not suited for DI:
</p>
<ul>
  <li><p>It is not very good at error handling. If your database connection cannot be established, the error will be hard to trace.</p></li>
  <li><p>It does not handle the case of circular dependencies.</p></li>
  <li><p>It does not offer any configuration options, for example, for injecting your database password from a secure source.</p></li>
</ul>
    )
  )
}
